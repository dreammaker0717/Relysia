...
export default async function topup(req, res) {
  ...
  const hmac_base64 = Buffer.from(hmac_raw).toString('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=/g, '');

  const url = `https://staging-topup.centi.ch/#/?client_id=${CLIENT_ID}&paymail_address=${encodeURIComponent(PAYMAIL)}&email=${encodeURIComponent(EMAIL)}&hmac=${hmac_base64}`;
